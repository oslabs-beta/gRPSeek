...
function findService(grpcObject: Record<string, any>, serviceName: string): Record<string, any> | null {
...
if (options.config) {
  if(typeof options.config === 'string'){
    ...
    const yamlConfig: IYamlConfig = yaml.load(fs.readFileSync(configPath, 'utf8')) as IYamlConfig;
...
// };
console.log('client',client[methodName])
...
console.log('service: kenlog ', service, 'methodname: ', methodName)
engine.addCall(client[methodName ?? ''].bind(client), payload, {}, callback, 1000);  
